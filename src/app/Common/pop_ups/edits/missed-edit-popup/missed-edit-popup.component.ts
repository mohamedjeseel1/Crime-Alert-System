...
export class MissedEditPopupComponent implements OnInit {
  ...
  constructor(private service:MissedService, @Inject(MAT_DIALOG_DATA) public data:any) { }
  ...
  updateMissed(){
    console.log("clicked")
    this.service.updateMissed(this.missedDetails.value, this.data.missed.id).subscribe((data)=>{
